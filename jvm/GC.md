...
一种复合算法，不是全新的算法，是将三种基础算法取长补短整合起来。不同的代的堆区域使用不同的垃圾回收算法。
...
1. 初始标记
   ...
   通过GC root找到直接可达对象，这个阶段stopword【stw】

2. 并发标记

   从上一个阶段标记出的对象，标记所有可达对象

   与用户线程同时工作不会STW

   

   三色标记：标记算法在并发标记时会出现对象丢失问题。CMS垃圾收集器使用增量更新算法解决。

   增量标记【增量更新】算法：在并发标记过程中用户线程为某些对象新增对象引用，记录下来，在重新标记阶段对这些对象重新进行可达分析，避免出现对象丢失问题。
...
3. 重新标记
   ...
   上阶段标记的对象，可能有误差，需要进行重新标记修正，需要STW

   因为上阶段用户线程和垃圾标记是并发执行的，在标记过程中可能产生新的垃圾【浮动垃圾】，或者垃圾对象被复活【finalize】，使用

4. 并发清除

   删除垃圾对象，与用户线程并发执行。不需要stw
...
RSet：保存其它region对象对自己region中对象的引用



SATB标记算法【原始快照】解决并发标记时对象丢失问题。
...
- **低延迟**：ZGC 是一个低延迟的垃圾回收器，旨在尽量避免 **Stop-the-World** 停顿。即使在大内存环境下，ZGC 也能够控制停顿时间在毫秒级别。
- **并发回收**：ZGC 是一个 **并发、增量式的垃圾回收器**，它几乎不会暂停应用线程，且能够实现 **并行的垃圾回收**。
- **内存分配优化**：ZGC 采用了一些新的内存分配技术，如 **着色指针**，使得它在大规模堆内存下也能有效工作。
...
- 缺点
  ...
  - 目前相对较新，某些场景下可能性能不如 G1 或 CMS。
  - 调优较为复杂。

##### 使用场景

- 对低延迟要求极高的应用，如金融、在线交易平台等。
- 大型内存（TB 级别）的系统。

##### 调优参数

- `-XX:+UseZGC`：启用 ZGC。
- `-XX:ZCollectionInterval=<n>`：设置每次垃圾回收的时间间隔。
- `-XX:ZGCHeapSize=<size>`：设置 ZGC 堆的大小。