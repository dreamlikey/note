...
进程无法直接操作IO设备的，其必须通过操作系统调用内核协助完成I/O动作，内核会为每个IO设备维护一个buffer内核缓冲区。
...
一个线程监听多个FD，并在某个FD可读可写时通知用户线程，从而避免无效等待，充分利用CPU资源
...
![多路复用IO执行过程](image/多路复用IO执行过程.jpg)
